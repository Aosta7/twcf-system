...
df["起始單號"] = df["起始單號"].str.strip()
df["結束單號"] = df["結束單號"].str.strip()
    ...
    if raw_input.strip() == "":
        ...
        for search_clean in input_list:
            ...
            if search_clean.isdigit():
                val = int(search_clean)
                
                # 逐筆檢視維護表內的區間
                for _, row in df.iterrows():
                    start_str = str(row["起始單號"]).strip()
                    end_str = str(row["結束單號"]).strip()

                    if start_str.isdigit() and end_str.isdigit():
                        # 條件：字串長度相同 且 數值介於起始與結束號碼之間
                        if len(search_clean) == len(start_str) and int(start_str) <= val <= int(end_str):
                            matched_row = row
                            break
            ...
            if matched_row is not None:
                results.append({
                    "輸入單號": search_clean,
                    "識別狀態": "✅ 成功",
                    "派件廠商": matched_row["派件廠商"],
                    "客戶代號 (客代)": matched_row["客戶代號(客代)"],
                    "雲端 API 密鑰狀態": "已就緒 (有Token)" if matched_row["黑貓API授權碼"] else "未配置",
                    "合約備註": matched_row["財務/合約備註"]
                })
            else:
                results.append({
                    "輸入單號": search_clean,
